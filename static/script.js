			...
			fetch(`/project/${project_id}`, {
				...
				.then(() => {
                    ...
                    <li class="list-group-item">
                        <h3>${data.name}</h3>
                        <p><span>Start</span>: ${data.start_date}</p>
                        <p><span>End</span>: ${data.end_date}</p>
                        <a class="completed_toggle btn btn-outline-danger" href="#" data-project="${project_id}" data-completed="false" data-id="${data.id}">Not Completed</a>
                        <a class="btn btn-outline-secondary" href="/edit_phase/${data.id}/${project_id}">Edit Phase</a>    
                    </li>
                `;

							document
								.querySelector('.list-group-flush')
								.insertAdjacentHTML('beforeend', phase);
							addListeners();
						});
				})
				.then(() => {
...
function setAvatar(color) {
	return `https://eu.ui-avatars.com/api/?bold=true?background=${color}`;
}
